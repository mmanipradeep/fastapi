...
def test_get_course_by_id_exists():
    response = client.get("/courses/65c8f119b40e9c02175da368")
    assert response.status_code == 200
    course = response.json()
    # get the course from the database
    course_db = db.courses.find_one({'_id': ObjectId('65c8f119b40e9c02175da368')})
...
def test_get_chapter_info():
    response = client.get("/courses/65c8f119b40e9c02175da368/1")
    assert response.status_code == 200
...
def test_get_chapter_info_not_exists():
    response = client.get("/courses/65c8f119b40e9c02175da368/990")
    assert response.status_code == 404
    assert response.json() == {'detail': 'Chapter not found'}
    
    
def test_rate_chapter():
    course_id = "65c8f119b40e9c02175da368"
    chapter_id = "1"
    rating = 1
